...
For our examples, we are going to be using eye tracking data from normal hearing individuals and those with cochlear impalnts using the Visaul Word Paradigm (VWP). 

```{r}
head(cohort_unrelated)
```

The `bdotsFit` function will create curve for each unique permutation of subject group variable. Here, we will let LookType and DB_cond be our grouping variables, though we may include as many as we wish (or only a single group assuming that it has multiple values). See `?bdotsFit` for argument information.
...
A key thing to note here is the argument for `curveType` is passed as a function with arguments that further specify the curve. Currently within the bdots package, the available curves are `doubleGauss(concave = TRUE/FALSE),` `logistic()` (no arguments), and `polynomial(degree = n)`. While more curves will be added, users can also specify their own curves, as shown [here](customCurves.html).
...
The plots for this object will compare the observed data with the fitted code. Here is an example of the first four
...
1. Difference of Curves
  : Here, we are interested specifically in the difference between two fitted curves. For our example case here, this may be the difference between curves for DB_cond 50 and 65 nested within either the Cohort or Unrelated_Cohort LookTypes (but not both). 
...
With our working example, we would find the difference of `DB_cond` `50` and `65` within `LookType == "Cohort"` with
...
The the second type of curve, we specify an "inner difference" to be the difference of groups for which we are taking the difference of. The syntax for this case uses a `diffs` function in the formula:
