...
For our examples, we are going to be using eye tracking data from normal hearing individuals and those with cochlear implants using data from the Visaul Word Paradigm (VWP). 

```{r}
head(cohort_unrelated)
```

The `bdotsFit` function will create curve for each unique permutation of `subject`/`group` variables. Here, we will let `LookType` and `DB_cond` be our grouping variables, though we may include as many as we wish (or only a single group assuming that it has multiple values). See `?bdotsFit` for argument information.
